...
public class LoggingInterceptor implements Interceptor {
	...
	public LoggingInterceptor() {
		withLogger(LoggerFactory.getLogger(LoggingInterceptor.class));
		
	}

	public  LoggingInterceptor withLogger(String name) {
		return withLogger(LoggerFactory.getLogger(name));
	}

	public  LoggingInterceptor withLogger(Class claz) {
		return withLogger(LoggerFactory.getLogger(claz));
	}

	public  LoggingInterceptor withLogger(Logger logger) {
		 this.slf4j = logger;
		 return this;
	}
	...
	public LoggingInterceptor withLevel(Level level) {
		if (level == null)
	...
	public boolean isLoggingDisabled() {
		if (level==null || level==Level.NONE || (!slf4j.isDebugEnabled())) {
			return true;
		}
		return false;
	}
	...
	public Response intercept(Chain chain) throws IOException {
		...
		try {
			...
			LoggerFactory.getLogger(LoggingInterceptor.class).warn("problem logging request: " + e); // no
																										// stack
																										// trace
		}
		...
		try {
			...
			if (logHeaders) {
				...
				if (!logBody || !HttpEngine.hasBody(response)) {
					...
					source.request(maxPrintableBodySize); // Buffer the entire
															// body.
